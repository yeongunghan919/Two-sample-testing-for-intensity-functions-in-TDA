...
def Aggtest(
    ...
    kernel: str
        The value of kernel must be "gaussian", "laplace"
    ...
    -------
    output : int
        0 if the aggregated Aggtest fails to reject the null
            (i.e. data comes from the same distribution)
        1 if the aggregated Aggtest rejects the null
            (i.e. intensity functions are different)
    dictionary: dict
        Returned only if return_dictionary is True.
        Dictionary containing the overall output of the Agg test.
    ...
    else: #optimal bandwidth collection
...
def Linear(diagram_1, diagram_2, vec_weight_1, vec_weight_2,kernel,bandwidth):
    """
    diagram 1, 2: Input diagrams, np.array of shape (n,2) 
    vec_weight_1 , 2: List of length n consisting of weight values of points in diagram 1 ,2.
    kernel : String representing the kernel function 
    bandwidth : Flaot , bandwidth of kernel function  
    
    return : Linear kernel value between diagram 1 and diagram 2 
    """
    if kernel=='gaussian':
...
def function_weight(name_weight, arc_c=1.0, arc_p=5.0, lin_el=1.0,poly_order=2):
    if name_weight == "arctan":
        ...
        def func_weight(bd):
            #return np.maximum(np.maximum((bd[1] - bd[0]) / lin_el, 1.0), 0.0)
            return (bd[1]-bd[0])
    elif name_weight == "Poly":
        def func_weight(bd):
            #return np.maximum(np.minimum((bd[1]-bd[0])**poly_order,1.0),0.0)
...
def Mat_gram(list_pd,bandwidth,weight_function,kernel,seed,Rff_approx=False,num_rff=10**4):
    """
    list_pd: List of length (n+m) which contains all input persistence daigrams
    kernel : String representing the kernel function 
    bandwidth : Flaot , bandwidth of kernel function  
    weight_function : Function , weight_function for computing weight of each points in diagram.
    Rff_approx : Boolean, 1 -> Random Fourier Feature approximation for computing Linear kernel value 
    num_rff : The number of samples for Rff approximation
    
    return : N * N matrix whose each element is Linear kernel value between corr two diagrams. 
    """
    num_pd = len(list_pd)
